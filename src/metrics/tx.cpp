...
namespace metrics {
    std::unique_ptr<TxMetrics> TxMetrics::make(const std::string &chain, prometheus::Registry& registry, bool noop) {
        if (noop) {
            return std::make_unique<TxMetrics>();
        }
        auto m = std::make_unique<TxMetricsImpl>(chain, registry);
        return reinterpret_cast<std::unique_ptr<TxMetrics> &&>(m);
    }
    TxMetricsImpl::TxMetricsImpl(const std::string& chain, prometheus::Registry& registry): Metrics(chain, registry) {
        ...
        auto& transactions_family = FamilyCounter("transactions");
