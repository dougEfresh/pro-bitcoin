...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  DOCKER_EXEC "update-alternatives --install /usr/bin/clang++ clang++ \$(which clang++-13) 100"
  DOCKER_EXEC "update-alternatives --install /usr/bin/clang clang \$(which clang-13) 100"
  DOCKER_EXEC "mkdir -p ${BASE_SCRATCH_DIR}/msan/build/"
  DOCKER_EXEC "git clone --depth=1 https://github.com/llvm/llvm-project -b llvmorg-13.0.0 ${BASE_SCRATCH_DIR}/msan/llvm-project"
